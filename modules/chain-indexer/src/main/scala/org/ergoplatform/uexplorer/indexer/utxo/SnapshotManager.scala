...
class SnapshotManager(
  ...
  def latestSerializedSnapshot: Option[Snapshot.Serialized] =
    if (snapshotDir.exists()) {
      val snapshots = snapshotDir.listFiles().collect {
        case file if file.getName.toIntOption.nonEmpty => Snapshot.Serialized(file.getName.toInt, file)
      }
      snapshots.sortBy(_.epochIndex).lastOption
    } else None

  def saveSnapshot(snapshot: Snapshot.Deserialized): Try[Unit] =
  ...
  def getLatestSnapshotByIndex: Try[Snapshot.Deserialized] =
    Try(latestSerializedSnapshot.get).map { case Snapshot.Serialized(latestEpochIndex, snapshotFile) =>
      ...
      try Snapshot.Deserialized(latestEpochIndex, objectInput.readObject.asInstanceOf[UtxoState])
      catch {
        case NonFatal(ex) =>
