...
if [ "$MEM_TOTAL" -lt 9000000 ]
then
    echo "Scylla is tested only with 16GB ram, you should have at least 9GB, it will most likely fail otherwise."
    exit 1
fi
...
docker run \
   ...
   -d scylladb/scylla:5.1 --overprovisioned 1 --developer-mode 1 --memory 6G --reserve-memory 3G --smp $SCYLLA_SMP
