...
SCYLLA_DOCKER_MEM="25G"
SCYLLA_MEM="22G"

while true; do
    read -p "Do you want to start scylla for initial sync (s) or polling (p)? " sp
    case $sp in
        [Ss]* )
          break;;
        [Pp]* )
          SCYLLA_DOCKER_MEM="8G"
          SCYLLA_MEM="5G"
          break;;
        * ) echo "s/p ?";;
    esac
...
if [ "$MEM_TOTAL" -lt 32000000 ]
then
    echo "Scylla is tested only with 32GB ram, it will most likely fail otherwise."
    exit 1
fi
...
docker run \
   ...
   -d scylladb/scylla:5.1 --overprovisioned 1 --developer-mode 1 --memory $SCYLLA_MEM --reserve-memory 3G --smp $SCYLLA_SMP
