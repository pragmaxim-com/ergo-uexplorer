...
object BoxRoutesSpec extends ZIOSpecDefault {
  ...
  def spec = suite("BoxRoutesSpec")(
    ...
    test("get spent/unspent/any box(es) by address / ergo-tree / ergo-tree-hash") {
      ...
      val spentByErgoTreeGet       = Request.get(URL(Root / "boxes" / "spent" / "ergo-trees" / Protocol.Emission.ergoTreeHex))
      val unspentByErgoTreeGet     = Request.get(URL(Root / "boxes" / "unspent" / "ergo-trees" / Protocol.Emission.ergoTreeHex))
      val anyByErgoTreeGet         = Request.get(URL(Root / "boxes" / "any" / "ergo-trees" / Protocol.Emission.ergoTreeHex))
      val spentByErgoTreeHashGet   = Request.get(URL(Root / "boxes" / "spent" / "ergo-tree-hashes" / Protocol.Emission.ergoTreeHash))
      val unspentByErgoTreeHashGet = Request.get(URL(Root / "boxes" / "unspent" / "ergo-tree-hashes" / Protocol.Emission.ergoTreeHash))
      val anyByErgoTreeHashGet     = Request.get(URL(Root / "boxes" / "any" / "ergo-tree-hashes" / Protocol.Emission.ergoTreeHash))
