...
object ExampleService {
  ...
  trait ExampleService {
    def getCharacters(name: String): UIO[List[Character]]
  ...
  def getCharacters(name: String): URIO[ExampleService, List[Character]] =
    ZIO.serviceWithZIO(_.getCharacters(name))
  ...
  def make(initial: List[Character]): ZLayer[Any, Nothing, ExampleService] = ZLayer {
    for {
      ...
      def getCharacters(name: String): UIO[List[Character]] =
        characters.get.map(_.filter(c => c.name == name))
