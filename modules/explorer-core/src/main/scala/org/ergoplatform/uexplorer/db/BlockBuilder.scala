...
object BlockBuilder {
  ...
  def apply(apiBlock: ApiFullBlock, prevBlock: Option[BlockMetadata], storage: Storage, backendEnabled: Boolean)(implicit
    ...
    val outputLookup =
      apiTransactions.transactions.iterator.flatMap(tx => tx.outputs.map(o => (o.boxId, (o.address, o.value)))).toMap
    val boxesByTx =
    ...
    BlockInfoBuilder(apiBlock, prevBlock).map { blockInfo =>
      ...
      if (!backendEnabled) {
        ...
        BestBlockInserted(lightBlock, Some(fullBlock))
      }
    }
  }
}
...
case class BestBlockInserted(lightBlock: LightBlock, fullBlockOpt: Option[FullBlock]) extends Inserted
