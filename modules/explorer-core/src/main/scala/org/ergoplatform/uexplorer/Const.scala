...
object Const {
  ...
  object Protocol {
    ...
    object Emission {
      ...
      val ergoTreeHex: ErgoTreeHex = ErgoTreeHex.fromStringUnsafe(
      ...
      val ergoTreeHash: ErgoTreeHash =
        ErgoTreeHash.fromStringUnsafe(Base16.encode(Sha256.hash(Base16.decode(ergoTreeHex.unwrapped).get)))

      val ergoTreeT8Hex: ErgoTreeT8Hash = ErgoTreeT8Hex.fromStringUnsafe(
      ...
      val ergoTreeT8Hash =
        ErgoTreeT8Hash.fromStringUnsafe(Base16.encode(Sha256.hash(Base16.decode(ergoTreeT8Hex.unwrapped).get)))
    ...
    object NoPremine {
      ...
      val ergoTreeHex: ErgoTreeHex = ErgoTreeHex.fromStringUnsafe("10010100d17300")
      val ergoTreeHash: ErgoTreeHash =
        ErgoTreeHash.fromStringUnsafe(Base16.encode(Sha256.hash(Base16.decode(ergoTreeHex.unwrapped).get)))
    ...
    object FeeContract {
      ...
      val ergoTreeHex: ErgoTreeHex = ErgoTreeParser.ergoTreeToHex(ErgoScriptPredef.feeProposition(MinerRewardDelta))
      val ergoTreeHash: ErgoTreeHash =
        ErgoTreeHash.fromStringUnsafe(Base16.encode(Sha256.hash(Base16.decode(ergoTreeHex.unwrapped).get)))
